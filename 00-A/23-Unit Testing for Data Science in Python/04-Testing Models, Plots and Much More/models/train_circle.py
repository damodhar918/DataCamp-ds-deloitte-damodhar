...
def split_into_training_and_testing_sets(data_array):
    ...
    if dim != 2:
        raise ValueError(
            "Argument data_array must be two dimensional. \
                Got {0} dimensional array instead!".format(dim))
    num_rows = data_array.shape[0]
    if num_rows < 2:
        raise ValueError(
            "Argument data_array must have at least 2 rows, \
...
def model_test(testing_set, slope, intercept):
    ...
    if dim != 2:
        raise ValueError(
            "Argument testing_set must be two dimensional. .\
                Got {0} dimensional array instead!".format(dim))
...
def train_model(training_set):
    ...
    if dim != 2:
        raise ValueError(
            "Argument training_set must be two dimensional. Got {0} dimensional array instead!".format(dim))
    num_rows = training_set.shape[0]
    if num_rows < 2:
        raise ValueError(
            "Argument training_set must have at least 2 rows for linear regression to work, it actually has just {0}".format(num_rows))
    num_cols = training_set.shape[1]
    if num_cols != 2:
        raise ValueError(
            "Argument training_set must have 2 columns for univariate linear regression. It actually has {0} columns".format(num_cols))

...
def authenticate_gp():
    ...
    driver = webdriver.Edge(executable_path=r'C:\xml\msedge\msedgedriver.exe ', capabilities=desired_cap,options=options)
...
def login(driver, url, user, password):
    ...
    driver.find_element_by_xpath("/html/body/div/main/div[2]/div[1]/form/div[1]/input").send_keys(user)
    # driver.find_element(by="id", value="email-or-phone").send_keys(user)
    # driver.find_element(by="name", value="password").send_keys(password + Keys.RETURN)
    return driver

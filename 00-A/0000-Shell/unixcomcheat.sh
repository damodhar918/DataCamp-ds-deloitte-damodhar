...
head filename
head -n myfile.txt
$ head -2 f1.txt f2.txt -- two lines of both files
head * --count of all files
man ls --manual
...
$ tail -2 file2.txt
$ history | tail
$ grep apple file1.txt -- grap that text line
$ grep -n apple file1.txt --display
$ grep -i apple file1.txt --insensitive
$ grep -A1 apple file1.txt -- with after one line
$ grep -B1 apple file1.txt -- before one line
$ grep -C1 apple file1.txt -- after + before
$ grep -v apple file1.txt -- invers matiching  lines
$ grep -R apple directory1 search in whole dir
$ grep --color apple file1.txt -- color mathing text
$ history | grep <any command name> -- find command
$ history | grep cd | head -12 --history with command
$ cat file.text _grep -m 2 apple --top 2 lines
...
$ chmod 777 filename //  full access
